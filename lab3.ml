...
let valid_rgb (x: color) : color =
  ...
  | RGB (a, b, c) ->
    ...
    then RGB (a, b, c)
    else raise (Invalid_Color "Not a valid color")
  | Simple (a) -> Simple (a)
...
let convert_to_rgb (x: color) : color=
  match x with
  | RGB (a, b, c) -> RGB (a, b, c)
  | Simple (a) ->
      match a with
...
let new_child (nm: string) (cl: color) (dt: date) : family =
  let psn: person = {name = nm; favorite = cl; birthdate = dt;} in
  Single(psn);;
...
let marry (fm : family) (psn : person) : family=
  match fm with
  | Single psn1 -> Family (psn1, psn, [])
  | Family -> raise (Family_Trouble "ouch")
 ;;
